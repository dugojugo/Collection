...
async function merge(fromPath, toPath, options = { bindings: {} }) {
    await rootWalk.walk(fromPath, async (_err, pathname, dirent) => {
        ...
        if (relPath.includes('.template')) {
            writeFileSync(join(toPath, toRelPath.replace('.template', '')), Handlebars.compile(readFileSync(pathname, 'utf8'))(options.bindings));
        }
...
export async function wrapScrapyProject({ projectPath }) {
    if (!projectPath) {
        projectPath = '.';
    }
...
//# sourceMappingURL=wrapScrapyProject.js.map