...
export const createPrefilledInputFileFromInputSchema = async (actorFolderDir) => {
    ...
    try {
        ...
        if (inputSchema) {
            ...
            // 		: fieldSchema.prefill,
            // );
            inputFile = Object.entries(inputSchema.properties).reduce((acc, [key, fieldSchema]) => {
                acc[key] =
                    fieldSchema.type === 'boolean' || fieldSchema.editor === 'hidden'
                        ? fieldSchema.default
                        : fieldSchema.prefill;
        ...
        warning({
            message: `Could not create default input based on input schema, creating empty input instead. Cause: ${err.message}`,
        });
    }
    finally {
...
export const getAjvValidator = (inputSchema, ajvInstance) => {
    const copyOfSchema = deepClone(inputSchema);
    copyOfSchema.required = [];
    for (const [inputSchemaFieldKey, inputSchemaField] of Object.entries(inputSchema.properties)) {
