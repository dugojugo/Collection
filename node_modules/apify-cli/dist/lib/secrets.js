...
export const removeSecret = (name) => {
    const secrets = getSecretsFile();
    if (!secrets[name])
        throw new Error(`Secret with name ${name} doesn't exist.`);
    delete secrets[name];
 ...
 */
export const replaceSecretsValue = (env, secrets) => {
 ...
 * @param secrets - Object with secrets, if not set, will be load from secrets file.
 */
export const transformEnvToEnvVars = (env, secrets) => {
    secrets = secrets || getSecretsFile();
    const envVars = [];
    Object.keys(env).forEach((key) => {
        if (isSecretKey(env[key])) {
