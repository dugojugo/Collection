...
const promisifiedTrack = promisify(mixpanel.track.bind(mixpanel));
/**
...
export const isTelemetryEnabled = !process.env.APIFY_CLI_DISABLE_TELEMETRY || ['false', '0'].includes(process.env.APIFY_CLI_DISABLE_TELEMETRY);
/**
 * Tracks telemetry event if telemetry is enabled.
 */
export const maybeTrackTelemetry = async ({ eventName, eventData, }) => {
 ...
 */
export const useApifyIdentity = async (userId) => {
    if (!isTelemetryEnabled)
        return;
    try {
        const distinctId = getOrCreateLocalDistinctId();
