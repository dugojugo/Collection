...
import { CommandExitCodes, DEFAULT_LOCAL_STORAGE_DIR, LEGACY_LOCAL_STORAGE_DIR, MINIMUM_SUPPORTED_PYTHON_VERSION, SUPPORTED_NODEJS_VERSION, } from '../lib/consts.js';
...
import { Ajv, checkIfStorageIsEmpty, getLocalInput, getLocalKeyValueStorePath, getLocalStorageDir, getLocalUserInfo, isNodeVersionSupported, isPythonVersionSupported, purgeDefaultDataset, purgeDefaultKeyValueStore, purgeDefaultQueue, } from '../lib/utils.js';
export class RunCommand extends ApifyCommand {
    static description = `Executes Actor locally with simulated Apify environment variables.\n` +
            ...
            switch (type) {
                case ProjectLanguage.JavaScript: {
                    ...
                    if (isNodeVersionSupported(runtime.version)) {
                        ...
                        warning({
                            message: `You are running Node.js version ${runtime.version}, which is no longer supported. ` +
                    ...
                    if (runType === RunType.DirectFile || runType === RunType.Module) {
                        ...
                        if (!runtime.pmPath) {
                            throw new Error('No npm executable found! Please make sure your Node.js runtime has npm installed if you want to run package.json scripts locally.');
                        }
                        await execWithLog({
                            cmd: runtime.pmPath,
                            args: ['run', entrypoint],
     ...
     */
    async validateAndStoreInput(inputOverride) {
        ...
        if (inputOverride) {
            ...
            if (errors.length > 0) {
                throw new Error(`${errorHeader}${errors
