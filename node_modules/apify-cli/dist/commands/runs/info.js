...
export class RunsInfoCommand extends ApifyCommand {
    static description = 'Prints information about an Actor run.';
    static args = {
        runId: Args.string({
            required: true,
    ...
    async run() {
        ...
        const message = [`${chalk.yellow('Actor')}: ${fullActorName} (${chalk.grey(run.actId)})`];
        // Handle task
        if (task) {
        ...
        if (defaultRequestQueue) {
            message.push(`${chalk.yellow('Requests')}: ${chalk.cyan(defaultRequestQueue.handledRequestCount.toLocaleString('en-US'))} out of ${chalk.cyan(defaultRequestQueue.totalRequestCount.toLocaleString('en-US'))} handled`);
        }
        else {
        ...
        if (run.finishedAt) {
            message.push(`${chalk.yellow('Finished')}: ${TimestampFormatter.display(run.finishedAt)} (took ${chalk.gray(ShortDurationFormatter.format(run.stats.durationMillis))})`);
        }
        else {
        ...
        if (run.stats.resurrectCount) {
            message.push(`${chalk.yellow('Resurrected')}: Yes, ${chalk.cyan(run.stats.resurrectCount.toLocaleString('en-US'))} ${this.pluralString(run.stats.resurrectCount, 'time', 'times')}`);
        }
        else {
        ...
        if (actor) {
            ...
            const runVersionTaggedAs = Object.entries((actor.taggedBuilds ?? {})).find(([, data]) => data.buildNumber === run.buildNumber)?.[0];
            ...
            if (actorVersion) {
                messageParts.push(`| ${chalk.gray('Actor version:')} ${chalk.cyan(actorVersion.versionNumber)} (${chalk.yellow(actorVersion.buildTag)})`);
            }
        ...
        message.push('');
        // CPU avg/max
        message.push(`${chalk.yellow('CPU')}: ${chalk.gray('Average:')} ${run.stats.cpuAvgUsage.toFixed(2)}% | ${chalk.gray('Maximum:')} ${run.stats.cpuMaxUsage.toFixed(2)}%`);
        // memory avg/max
        message.push(`${chalk.yellow('Memory')}: ${chalk.gray('Average:')} ${prettyPrintBytes({ bytes: run.stats.memAvgBytes, shortBytes: true })} | ${chalk.gray('Maximum:')} ${prettyPrintBytes({ bytes: run.stats.memMaxBytes, shortBytes: true })}`);
        // url
        message.push('');
