...
import { CommandExitCodes, DEFAULT_LOCAL_STORAGE_DIR, EMPTY_LOCAL_CONFIG, LOCAL_CONFIG_PATH, PROJECT_TYPES, } from '../lib/consts.js';
...
export class InitCommand extends ApifyCommand {
    static description = `Sets up an Actor project in your current directory by creating actor.json and storage files.\n` +
    ...
    async run() {
        ...
        if (actorConfig.isOkAnd((cfg) => cfg.exists && !cfg.migrated)) {
            ...
            if (!actorName) {
                let response = actorConfig.isOk() ? { actName: actorConfig.unwrap().config.name } : null;
                while (!response) {
                    try {
