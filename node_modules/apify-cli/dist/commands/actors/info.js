...
export class ActorsInfoCommand extends ApifyCommand {
    static description = 'Get information about an Actor.';
    static flags = {
    ...
    async run() {
        ...
        if (actorInfo.actorMaker) {
            message.push('', `${chalk.yellow('Made by:')} ${chalk.cyan(actorInfo.actorMaker.profile.name ?? actorInfo.actorMaker.username)}`);
            // Missing types who?
        ...
        // Pricing info
        const pricingInfo = Reflect.get(actorInfo, 'pricingInfos');
        if (pricingInfo?.length) {
            ...
            switch (latestPricingInfo.pricingModel) {
                case 'FLAT_PRICE_PER_MONTH': {
                    message.push(`${chalk.yellow('Pricing information:')} ${chalk.bgGray(`$${latestPricingInfo.pricePerUnitUsd}/month + usage`)}`);
                ...
                case 'PRICE_PER_DATASET_ITEM': {
                    ...
                    message.push(`${chalk.yellow('Pricing information:')} ${chalk.bgGray(`$${pricePerOneKItems.toFixed(2)} / 1,000 results`)}`);
                    break;
                }
                case 'PAY_PER_EVENT': {
                ...
                default: {
                    message.push(`${chalk.yellow('Pricing information:')} ${chalk.bgGray(`Unknown pricing model (${chalk.yellow(latestPricingInfo.pricingModel)})`)}`);
                }
            }
        }
        else {
        ...
        if (actorInfo.taggedBuilds) {
            ...
            if (latestBuild) {
                message.push(`  ${chalk.yellow('-')} ${chalk.cyan(latestBuild.buildNumber)} ${chalk.gray('/')} ${chalk.yellow('latest')}`);
            }
            for (const [buildTag, build] of Object.entries(actorInfo.taggedBuilds)) {
                if (buildTag === 'latest') {
                    continue;
                }
                message.push(`  ${chalk.yellow('-')} ${chalk.cyan(build.buildNumber)} ${chalk.gray('/')} ${chalk.yellow(buildTag)}`);
            }
        }
