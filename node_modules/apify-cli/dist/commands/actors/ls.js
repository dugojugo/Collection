...
import { DateOnlyTimestampFormatter, getLoggedClientOrThrow, MultilineTimestampFormatter, ShortDurationFormatter, } from '../../lib/utils.js';
const statusMap = {
...
export class ActorsLsCommand extends ApifyCommand {
    static description = 'Prints a list of recently executed Actors or Actors you own.';
    static flags = {
        my: Flags.boolean({
    ...
    async run() {
        ...
        const actorList = {
            ...rawActorList,
            items: await Promise.all(rawActorList.items.map(async (actorData) => {
        ...
        const longestActorTitleLength = actorList.items.reduce((acc, curr) => {
        ...
        for (const item of actorList.items) {
            ...
            const lastRunDuration = item.lastRun
                ? (() => {
                    if (item.lastRun.finishedAt) {
                        return ShortDurationFormatter.format(item.lastRun.finishedAt.getTime() - item.lastRun.startedAt.getTime());
                    }
            ...
            const defaultBuild = item.actor
                ? (() => {
                    const buildVersionToTag = Object.entries((item.actor.taggedBuilds ?? {})).find(([tag, data]) => data.buildNumber === item.actor.defaultRunOptions.build ||
            ...
            table.pushRow({
                ...
                Runs: ResponsiveTable.isSmallTerminal() && longestActorTitleLength >= 56
                    ? kSkipColumn
