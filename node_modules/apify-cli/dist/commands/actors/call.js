...
export class ActorsCallCommand extends ApifyCommand {
    static description = 'Executes Actor remotely using your authenticated account.\n' +
    ...
    async run() {
        ...
        const { id: actorId, userFriendlyId, actorData, } = await ActorsCallCommand.resolveActorId({
            client: apifyClient,
            localActorName: localConfig.name,
            usernameOrId,
        ...
        for await (const yieldedRun of iterator) {
            ...
            if (!runStarted) {
                ...
                if (!this.flags.silent) {
                    ...
                    const runVersionTaggedAs = Object.entries((actorData.taggedBuilds ?? {})).find(([, data]) => data.buildNumber === yieldedRun.buildNumber)?.[0];
    ...
    static async resolveActorId({ client, localActorName, usernameOrId, providedActorNameOrId, }) {
        // Full ID
