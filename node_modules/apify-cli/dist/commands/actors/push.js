...
import { createActZip, createSourceFiles, getActorLocalFilePaths, getLocalUserInfo, getLoggedClientOrThrow, outputJobLog, } from '../../lib/utils.js';
...
export class ActorsPushCommand extends ApifyCommand {
    static description = `Deploys Actor to Apify platform using settings from '${LOCAL_CONFIG_PATH}'.\n` +
    ...
    async run() {
        ...
        const version = this.flags.version || actorConfig?.version || DEFAULT_ACTOR_VERSION_NUMBER;
        let buildTag = this.flags.buildTag || actorConfig?.buildTag;
        ...
        if (filesSize < MAX_MULTIFILE_BYTES) {
            ...
            if (!isActorCreatedNow) {
                ...
                if (!this.flags.force &&
                    ...
                    throw new Error(`Actor with identifier "${actorConfig?.name || forceActorId}" is already on the platform and was modified there since modified locally.
Skipping push. Use --force to override.`);
        ...
        const envVars = actorConfig.environmentVariables
            ? transformEnvToEnvVars(actorConfig.environmentVariables)
            : undefined;
